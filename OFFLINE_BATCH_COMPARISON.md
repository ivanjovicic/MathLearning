...
## ?? Konacno Rešenje: Hybrid Pristup
...
**Benefit**: Sigurnost - klijent ne može da lažira tacne odgovore.
...
```bash
# Pokušaj da lažiraš tacan odgovor
curl -X POST https://mathlearning-api.fly.dev/api/quiz/offline-submit \
  -H "Authorization: Bearer TOKEN" \
  -d '{
...
# Server ce markirat kao NETACAN (ignoriše isCorrectOffline)
```
...
- [ ] **Conflict resolution**: Šta ako isti odgovor postoji sa razlicitim `answeredAt`?
