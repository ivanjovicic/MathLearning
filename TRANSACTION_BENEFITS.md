...
- Ako dode do greške, `RollbackAsync()` vraca sve na staro
...
**Benefit**: Baza ostaje u validnom stanju cak i pri greškama.
...
group.MapPost("/offline-submit", async (request, db, ctx) =>
{
    ...
    try
    {
        ...
        await trx.CommitAsync();     // Atomicki upiši sve odjednom
...
**Benefit**: Garantovana atomicnost - ili sve ili ništa.
...
UserQuestionStats: delimicno ažurirano
...
- **Brže od multiple savechanges** - jedna transakcija je brža od N pojedinacnih upisa
...
- **Rizican** - može videti nevalidne podatke
...
1. ? Garantuju atomicnost - sve ili ništa
