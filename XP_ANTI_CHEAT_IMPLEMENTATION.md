...
## ?? Overview
Implementiran je sistem dodeljavanja XP-a koji sprecava varanje i prati sve odgovore korisnika.
...
### 2. **Anti-cheat logika**
- XP se dodeljuje **samo prvi put** kada korisnik tacno reši pitanje
- `UserQuestionStat.CorrectAttempts == 0` ? dodeli XP
- Svaki sledeci tacan odgovor ? 0 XP (vec nagradeno)
...
3. `src/MathLearning.Api/Endpoints/QuizEndpoints.cs` (submit answer endpoint)
   ...
   - Dodato: Provera da li je prvi tacan odgovor
   - Dodato: Kreiranje/ažuriranje `UserProfile.Xp`
   - Dodato: Dodavanje zapisa u `UserAnswerAudits`
   - Vraca: Prošireni response sa XP podacima

## ?? Sledeci koraci (opciono)
...
4. **Achievements/Badges**: Nagrade za odredene milestones (npr. "100 XP earned")
...
- ? Audit log za sve odgovore (sprecava manipulaciju istorije)
- ? Unique index na `UserAnswers(UserId, QuestionId, AnsweredAt)` — sprecava duplikate
