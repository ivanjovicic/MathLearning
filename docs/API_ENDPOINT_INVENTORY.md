...
| POST | `/auth/password/forgot` | Public | Canonical | Generic 202 response for matching and non-matching email; rate limited by normalized email and network/device; delivery uses `IPasswordResetDelivery`. |
| POST | `/auth/password/reset` | Public | Canonical | Identity token reset; safe invalid/policy/rate-limit failures; successful reset rotates the security stamp and revokes all refresh tokens. |
