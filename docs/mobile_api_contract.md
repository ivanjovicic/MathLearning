...
For relational providers, reset commits the Identity password/security-stamp update and active refresh-token revocation in one scoped transaction. Identity 8.0.12's `ResetPasswordAsync` is the authoritative security-stamp rotation path; the backend does not perform a second stamp rotation. Production reset messages are dispatched through the existing Hangfire background-job path after the generic 202 response; Test and Development use deterministic inline delivery.
