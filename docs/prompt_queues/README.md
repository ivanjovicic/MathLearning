...
| 4 | `backend_test_coverage.md` | Provider/cancellation/regression proof that supports canonical runtime owners without reimplementing them; top unresolved bugs are `BACKEND-API-DB-015` and `BACKEND-TEST-033`. Design-token `BACKEND-TEST-050`/`BACKEND-TEST-051` and authoring `BACKEND-TEST-049` runtime slices delivered with residuals deferred. |
