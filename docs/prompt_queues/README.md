...
| 4 | `backend_test_coverage.md` | Provider/cancellation/regression proof that supports canonical runtime owners without reimplementing them; top unresolved bugs are `BACKEND-API-DB-015`, `BACKEND-TEST-033` and `BACKEND-TEST-050`/`BACKEND-TEST-051`. `BACKEND-TEST-049` runtime slice delivered (UpdateQuestion split-save residual deferred). |
