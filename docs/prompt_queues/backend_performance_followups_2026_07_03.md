...
| BE-PERF-010 | P1 | Validated | Replace hourly all-profile XP reset work with set-based, cancellable and single-owner processing. Run log: `.ai/runs/2026-07-30-BE-PERF-010-evidence.md`. Validated against Docker PostgreSQL on `localhost:5433` with `XpResetProcessorTests` passing `10/10`. |
| BE-PERF-011 | P1 | Validated | Bound rate-limit memory/cardinality, accurate `Retry-After`, proxy-aware client identity and per-replica single-node semantics. Run log: `.ai/runs/2026-07-30-BE-PERF-011-evidence.md`; tests passed `19/19`. |
...
| BE-PERF-013 | P1 | Done 79% — Run log: `.ai/runs/2026-07-31-BE-PERF-013-evidence.md`; Validation: UserSettings+ReadPathMutation 17/17; Residual risk: cosmetics GET EnsureDefaultOwnership writes deferred; push/PR/main open; Commit: self | Remove refresh, snapshot, streak and reward mutations from GET/read paths. |
