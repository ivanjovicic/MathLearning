...
Documentation impact: update one canonical contract to state whether milestone XP affects all-time/daily/weekly/monthly values, its source identity, replay semantics and atomic relationship with milestone claim state.
...
Validation:
```powershell
python scripts/run_guarded.py --timeout-seconds 240 -- dotnet test tests/MathLearning.Tests/MathLearning.Tests.csproj -c Release --filter FullyQualifiedName~SeasonMilestone
