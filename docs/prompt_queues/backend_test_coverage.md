...
| BACKEND-TEST-026 | In progress / stale PR refresh required — PR #23 remains open but is ~115 commits behind current main; refresh from current main before any merge. | Minimize public health/metrics/schema/job information. |
...
| BACKEND-TEST-033 | In progress / stale PR refresh required — PR #24 carries partial practice cancellation proof but is ~115 commits behind current main; no duplicate claim. | Cancellation/rollback matrix for canonical P0 mutations; supporting proof for BE-PERF-012 and 015. |
...
| BACKEND-TEST-050 | In progress / stale PR refresh required — PR #20 remains open and ~115 commits behind current main; port and revalidate from current main. | Design-token draft version identity and same-second collision safety. |
| BACKEND-TEST-051 | In progress / stale PR refresh required — PR #21 remains open and ~115 commits behind current main; port and revalidate from current main. | Design-token bootstrap race and startup ownership. |
...
| BACKEND-API-DB-015 | P0/P1 / Needs validation — promoted to Ready and claimed 2026-09-19 on `agent/BACKEND-API-DB-015`; lease/takeover implementation delivered; PostgreSQL concurrency matrix remains | Recover stale pending economy/cosmetics operations without allowing dual settlement. |
