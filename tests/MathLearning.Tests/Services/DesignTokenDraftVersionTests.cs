...
public sealed class DesignTokenDraftVersionTests
{
    ...
    [Fact]
    public void DraftIdentity_UsesSingleDraftUniqueConstraint()
    {
        using var db = TestDbContextFactory.Create();
        var entityType = db.Model.FindEntityType(typeof(DesignTokenVersion));
        Assert.NotNull(entityType);

        var draftIndex = entityType!.GetIndexes()
            .Single(index => index.GetDatabaseName() == "UX_DesignTokenVersion_Draft");
